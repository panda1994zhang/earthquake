...
class WechatReports:
    ...
    def __init__(self, Config=None):
        ...
        self.PathList = {"token": "WeChatTokens", "tmp": "TmpHtml"}
    ...
    def GetLocalToken(self, AppId="main"):
        ...
        if str(AppId) in self.Conf["Apps"]:
            Token = ""
            Path = self.PathChange(self.Conf["TokenPath"].format(AppId))
            ...
            Token_Validity = self.Session.post(self.GetUrl("sendmsg", Token)).json()
            if "errcode" in Token_Validity and int(Token_Validity["errcode"]) in [
                40014,
                40001,
                42001,
            ]:
    ...
    def UpLoad(self, Token, Types, Data, Files):
        ...
        Res = self.Session.post(
            self.GetUrl("upload", Token), params={"type": Types}, data=Data, files=Files
        )
    ...
    def UpLoadPic(self, Token, FileRaw):
        """
                上传图像并获取media_id
        """
        Res = self.Session.post(self.GetUrl("uploadpic", Token), files=FileRaw)
        if self.SuccessResp(Res):
    ...
    def SendMsgNow(self, Token, Data):
        Resp = self.Session.post(self.GetUrl("sendmsg", Token), data=json.dumps(Data))
            ...
            Data.update(
                {
                    "file": {
                        "media_id": self.upload(
                            Token, MsgType, {"media": ""}, {"file": open(Content, "rb")}
                        ).get("media_id", "")
                    }
                }
            )
